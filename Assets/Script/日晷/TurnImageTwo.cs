...
public class TurnImageTwo : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    public Image imageToRotateTwo; // 需要旋转的Image组件
    ...
    public bool image2Reached90 = false;
    ...
    private void Start()
    {
        ...
        originalColor = imageToRotateTwo.color; // 存储原始颜色  
        currentAngle = imageToRotateTwo.rectTransform.localEulerAngles.z;
    }
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        // 获取点击的世界坐标
        Vector2 localPoint;
        if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(imageToRotateTwo.rectTransform, eventData.position, eventData.pressEventCamera, out localPoint))
        {
            return;
        }

        // 将本地坐标转换为UV坐标 (0-1)
        float u = (localPoint.x - imageToRotateTwo.rectTransform.rect.x) / imageToRotateTwo.rectTransform.rect.width;
        float v = (localPoint.y - imageToRotateTwo.rectTransform.rect.y) / imageToRotateTwo.rectTransform.rect.height;

        // 将UV坐标转换为像素坐标
        Sprite sprite = imageToRotateTwo.sprite;
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        imageToRotateTwo.rectTransform.rotation = rotationQuaternion;
        ...
        if (Mathf.Abs(currentRotation * Mathf.Rad2Deg - 0f) <= CLOSE_TO_90_DEGREES)
        {
            ...
            if (!isGreen)
            {
                ...
                imageToRotateTwo.color = lightGreen; // 设置图片为绿色  
                canDrag = false;
            }
            ...
            {
                isGreen = false;
                imageToRotateTwo.color = originalColor; // 恢复图片为原色  
            }
        }
        else
        {
            ...
            {
                isGreen = false;
                imageToRotateTwo.color = originalColor;
            }
        }
    }
